...
describe('Blog app', () => {
    ...
    describe('When logged in', () => {
        ...
        test('a new blog can be created', async ({ page }) => {
            ...
            const blog = await page.locator('.nav').first().locator('.nav-item').first()
            await blog.click()
            await expect(page.getByText('Test Title')).toBeVisible()
            await expect(page.getByText('test_user').nth(1)).toBeVisible()
            await expect(page.getByText('0')).toBeVisible()
            await expect(page.getByText('https://www.testBlogUrl.com')).toBeVisible()
        })
        ...
        test('a blog can be deleted', async ({ page }) => {
            ...
            await expect(page.getByText('Test Title')).not.toBeVisible({ timeout: 10000 })
        })
